...
CMD=$1
API_CONTAINER="riderecord-api"
export DOCKER_COMPOSE_ENV=${WORKSPACE_DIR}/setup/docker-compose.yml
...
if [[ "$CMD" == "up" ]] || [[ "$CMD" == "start" ]]; then
	...
	$WORKSPACE_DIR/setup/cli.sh logs api
	...
	if [[ -n "$container" ]] && [[ "$container" != "$API_CONTAINER" ]]; then
		container="riderecord-$container"
	fi

	printf "\e[92mListing \e[96m$container\e[92m logs\e[0m\n";

	docker logs $container --tail=${lines} -t -f | $WORKSPACE_DIR/setup/docker-logs-localtime
